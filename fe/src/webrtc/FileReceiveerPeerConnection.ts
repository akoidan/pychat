...
export default class FileReceiverPeerConnection extends FilePeerConnection {
  ...
  private async initFileSystemApi() {
    ...
    try {
      const fs: FileSystem = await new Promise<FileSystem>((resolve, reject) => requestFileSystem(
        window.TEMPORARY,
        this.fileSize,
        resolve,
        reject,
      ));
      this.fileEntry = await new Promise<FileEntry>((resolve, reject) => fs.root.getFile(this.connectionId, {create: true}, resolve, reject));
      this.fileWriter = await new Promise<FileWriter>((resolve, reject) => this.fileEntry!.createWriter(resolve, reject));
  ...
  private async clearFS(fs: FileSystem) {
    ...
    const entries: Entry[] = await new Promise<Entry[]>((resolve, reject) => fs.root.createReader().readEntries(resolve, reject));
