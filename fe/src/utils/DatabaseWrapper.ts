...
export default class DatabaseWrapper implements IStorage {
  ...
  public insertMessage(t: SQLTransaction, message: MessageModel) {
    ...
    this.executeSql(t, "insert or replace into message (id, time, content, symbol, deleted, giphy, edited, roomId, userId, sending) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [message.id, message.time, message.content, message.symbol ?? null, message.deleted ? 1 : 0, message.giphy ?? null, message.edited, message.roomId, message.userId, message.transfer ? 1 : 0], (t, d) => {
  ...
  private executeSql(
    ...
    const err: SQLStatementErrorCallback = e ? e : (t: SQLTransaction, e: SQLError) => {
      this.logger.error("{} {}, error: {}, message {}", sql, args, e, e.message)();
