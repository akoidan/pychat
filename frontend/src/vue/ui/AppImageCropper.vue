    ...
    <div class="imgHolder">
      <section class="cropper-area">
        ...
        <div v-if="srcImg" class="actions">
          <i title="Zoom In (Mouse Wheel)" class="icon-zoom-in" @click="cropper.relativeZoom(0.2)"/>
          <i title="Zoom Out (Mouse Wheel)" class="icon-zoom-out" @click="cropper.relativeZoom(-0.2)"/>
          <i title="Rotate" class="icon-rotate" @click="cropper.rotate(90)"/>
          <i title="Flip Y" class="icon-resize-horizontal" @click="cropper.scaleX(scaleY = scaleY * -1)"/>
          <i title="Flip X" class="icon-resize-vertical" @click="cropper.scaleY(scaleX = scaleX * -1)"/>
        </div>
      </section>
      <video
        v-show="showVideo"
...
export default class AppImageCropper extends Vue {
  ...
  public async startCapturingVideo() {
    try {
      ...
      this.srcVideo = await navigator.mediaDevices.getUserMedia({audio: true,
                                                                 video: true});
      this.showVideo = true;
