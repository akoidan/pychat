...
export default class AbstractMessageProcessor {
  ...
  protected readonly callBacks: { [id: number]: {resolve: Function; reject: Function} } = {};
  ...
  public setMessageCbId<T extends DefaultWsOutMessage<K>, K extends string>(message: T) {
    ...
    message.cbId = ++this.uniquePositiveMessageId; // should be ++, so uniqueId won't be 0, if it's 0 then checks like !0 are false, but should be true
  }
