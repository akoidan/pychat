...
CONF_VERSION='659a7950453576dea80bb28011253af61ea73c7a'
...
declare -a files=("$STATIC_DIR/favicon.ico" "$SOUNDS_DIR/ChatOutgoing.wav" "$SOUNDS_DIR/ChatIncoming.wav" "$SOUNDS_DIR/ChatLogin.wav" "$SOUNDS_DIR/ChatLogout.wav" "$CSS_DIR/pikaday.css" "$JS_DIR/pikaday.js" "$JS_DIR/moment.js" "$CSS_DIR/fontello.css" "$FONT_DIR/fontello.eot" "$FONT_DIR/fontello.svg" "$FONT_DIR/fontello.ttf" "$FONT_DIR/fontello.woff" "$FONT_DIR/fontello.woff2" "$SMILEYS_DIR" "$SMILEYS_DIR/info.json" "$SMILEYS_DIR/base/0000.gif" "$STATIC_DIR/ajaxStatus.gif" "$VAL_DIR/Love_and_Passion.ttf" "$VAL_DIR/teddy.gif" "$VAL_DIR/v2.ttf" "$VAL_DIR/videoplayback.mp3")
