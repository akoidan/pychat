...
window.USER_REGEX = /^[a-zA-Z-_0-9]{1,16}$/;
window.blankRegex = /^\s*$/;
var fileTypeRegex = /\.(\w+)(\?.*)?$/;
...
window.isFirefox = window.browserVersion.indexOf('Firefox') >= 0;
window.isChrome = window.browserVersion.indexOf('Chrome') >= 0;
...
RTCPeerConnection.RTCPeerConnection = window.mozRTCPeerConnection || window.webkitRTCPeerConnection || window.RTCPeerConnection;
...
function setTheme(theme) {
	...
	window.isMobile && CssUtils.addClass(document.body, 'mobile')
}
onDocLoad(function () {
	muteBtn = $("muteBtn");
